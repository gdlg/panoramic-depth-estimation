...
def bilinear_sampler_equirectangular(input_images, d_offset, fov, wrap_mode='border', name='bilinear_sampler', **kwargs):
    ...
    def _transform(input_images, d_offset):
        with tf.variable_scope('transform'):
            ...
            P = tf.stack(
                    [angular_precision, 0., _width_f/2.,
            ...
            d_t_flat = tf.reshape(d_offset, [-1])


            with tf.variable_scope('tolonlat'):
                xyone = tf.stack([x_t_flat, y_t_flat, tf.ones_like(x_t_flat)])
                lonlatone = tf.matmul(Pinv, xyone)
                lon = lonlatone[0]
                lat = lonlatone[1]


            with tf.variable_scope('toxyz'):
                # To (x,y,z)
                nt = tf.tan(lon)
                ny = tf.sin(lat)
                ...
                nx = tf.sqrt((1-ny*ny)/(1+nt*nt)) * nt
                nz = tf.sqrt(1 - nx*nx - nyny)
            ...
            with tf.variable_scope('backtolonlat'):
                lon = tf.atan(nx/nz)
                lat = tf.asin(ny/tf.sqrt(nx*nx + nyny + nz*nz))

            with tf.variable_scope('backtoxyflat'):
                lonlatone = tf.stack([lon, lat, tf.ones_like(lon)])

                xyone = tf.tensordot(P, lonlatone, 1)
